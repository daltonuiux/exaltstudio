...
/**
 * FloatingLines, from React Bits (https://reactbits.dev), ported to TypeScript
 ...
 * - If WebGL isn't available it renders nothing rather than throwing.
 * - Time comes from performance.now(), not THREE.Clock (deprecated in recent
 *   three, and it logged a console warning).
 * - `onReady` fires after the first frame is drawn, so the caller can fade it in.
 */
...
export type FloatingLinesProps = {
  /** Up to 8 hex colours the lines are graded through. */
  linesGradient?: string[];
  enabledWaves?: Wave[];
  /** A number applies to every wave; an array is per enabled wave. */
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // No WebGL: leave the sky as it is.
      return;
    }
    ...
    // The original re-creates the scene when any prop changes; same here.
